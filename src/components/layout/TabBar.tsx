...
import { Home, BookOpen, Timer, Sparkle, Settings } from "lucide-react";
...
export const TabBar: React.FC<TabBarProps> = ({ activeTab, onTabChange }) => {
  ...
  return (
    <>
      ...
      <nav className="fixed bottom-[calc(0.75rem+env(safe-area-inset-bottom))] left-1/2 -translate-x-1/2 w-[calc(100vw-32px)] max-w-[380px] z-50 pointer-events-none md:hidden">
        <div className="pointer-events-auto bg-black/40 backdrop-blur-xl border border-white/10 shadow-lg rounded-[2rem] flex items-center justify-between px-2 py-2">
          {tabs.map((tab) => {
            ...
            // Custom rendering for Timer tab on mobile
            const isTimerTab = tab.id === "timer";
            const showRunningTimer =
              isTimerTab && (timerIsActive || totalElapsed > 0);
              ...
              >
                {isActive && (
                  <motion.div
                    ...
                    className="absolute -top-[1px] w-8 h-[3px] bg-gold rounded-full shadow-[0_0_10px_rgba(245,200,66,0.8)]"
                ...
                {isActive && (
                  <motion.div
                    ...
                    className="absolute inset-[4px] bg-gold/10 rounded-[1.2rem] -z-10 blur-[2px]"
                  />
                )}
                <div
                  className={cn(
                    ...
                    isActive
                      ? "text-gold scale-110 drop-shadow-[0_0_8px_rgba(245,200,66,0.5)] mb-0.5"
                      : "text-white/30 hover:text-white/60 mb-1",
                  )}
                >
                  {showRunningTimer ? (
                    <span className="font-serif text-[13px] font-bold tracking-widest leading-none mt-1">
      ...
      <nav className="hidden md:flex fixed top-0 bottom-0 left-0 z-50 flex-col group w-[72px] hover:w-[240px] transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] bg-black/10 hover:bg-black/80 backdrop-blur-md hover:backdrop-blur-2xl border-r border-white/5 shadow-2xl py-10 px-3 overflow-y-auto overflow-x-hidden pointer-events-auto overscroll-contain">
        {/* App Logo/Icon */}
        <div className="flex items-center gap-3 px-0 mb-12">
          ...
          <div className="opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-x-[-10px] group-hover:translate-x-0 whitespace-nowrap">
            ...
            <div className="text-gold font-bold text-[10px] tracking-[0.2em] uppercase">
              Journal
            </div>
          </div>
        </div>
